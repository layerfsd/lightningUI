...
#elif defined _MSC_VER	//	add by Nanling for fast check instance
#  define _G_TYPE_CIT(ip, gt) \
	(((ip) && ((GTypeInstance*) ip)->g_class && ((GTypeInstance*) ip)->g_class->g_type == gt) \
		? TRUE : (g_type_check_instance_is_a ((GTypeInstance*) ip, gt)))
#  define _G_TYPE_CCT(cp, gt) \
	(((cp) && ((GTypeClass*) cp)->g_type == gt) \
		? TRUE : (g_type_check_class_is_a ((GTypeClass*) cp, gt)))
#  define _G_TYPE_CVH(vl, gt) \
	(((vl) && ((GValue*) vl)->g_type == gt) \
		? TRUE : (g_type_check_value_holds ((GValue*) vl, gt)))
#else  /* !__GNUC__ */
