...
szCmd的示例:
i_Mode = ISS_SR_MODE_LOCAL_CMDLIST时:
{
    "grm": [
        {
            "dictname": "cmd",
...
ISSErrID ISSAPI ISSSRStart(
    ...
    int         iMode,     /* [In] i_Mode = ISS_SR_MODE_CLOUD_REC,纯网络端识别;
                                   ...
                                   i_Mode = ISS_SR_MODE_LOCAL_CMDLIST,本地命令词识别
                                            ,此时忽略szScene参数;
                                   i_Mode = ISS_SR_MODE_LOCAL_NLP,纯本地语义,
                                            ,此时忽略szScene参数.
                            */
    const char* szCmd      /* [In] i_Mode = ISS_SR_MODE_LOCAL_CMDLIST时,
                                   为本地命令词识别所用命令词列表,json格式;
								   i_Mode = ISS_SR_MODE_LOCAL_NLP时,
