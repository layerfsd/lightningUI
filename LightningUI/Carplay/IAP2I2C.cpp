...
//static ULONG   g_ulDevAddr = IAP2_DEVADDR;
...
OSStatus IAP_I2C_Open()
{
    if (g_hI2CHandle == NULL)
    {
        g_hI2CHandle = I2COpen(IAP2_I2C_SYSCALL_PORT);
        g_dwOpenCount = 0;
    }
    if(g_hI2CHandle == NULL)
    {
...
OSStatus IAP_I2C_Write(UINT32 ulRegAddr, UINT8* pRegValBuf, UINT uiValBufLen)
{
	I2CTRANS  Trans;
    UINT      iRetry = 0;
    BOOL      bRet = FALSE;
	...
	Trans.dwClock = IAP2_I2C_W_CLOCK;
	Trans.dwPort = IAP2_NOSYSCALL_I2C_PORT;
	...
	Trans.i2cdevinfo.ulDevAddrW = IAP2_DEVADDR;
	...
	do
	{
		...
		{
			iRetry++;
			Sleep(kMFiI2cRetryDelayMics);
		}
	}while (iRetry<IA2P2_I2C_RETRY_TIME && !bRet);

    if(!bRet)
    {
        RETAILMSG(1,(TEXT("IAP_I2C_Write: I2C write [0x%X] failed\r\n"),ulRegAddr));
        return -6747;
    }
...
OSStatus IAP_I2C_Read(UINT32 ulRegAddr, UINT8* pRegValBuf, UINT uiValBufLen)
{
	I2CTRANS  Trans;
    UINT      iRetry=0;
    BOOL      bRet = FALSE;
	...
	Trans.pValueBuf = pRegValBuf;
	Trans.uiValueLen = uiValBufLen;
	Trans.dwClock = IAP2_I2C_R_CLOCK;
	Trans.dwPort = IAP2_NOSYSCALL_I2C_PORT;
	Trans.i2cdevinfo.pReadSeq = (PBYTE)g_pI2CReadSeq;
	Trans.i2cdevinfo.uiReadSeqLen = (UINT)sizeof(g_pI2CReadSeq);
	Trans.i2cdevinfo.pWriteSeq = (PBYTE)g_pI2CWriteSeq;
    ...
    if(!bRet)
    {
        RETAILMSG(1,(TEXT("IAP_I2C_Read: I2C read [0x%X] failed\r\n"),ulRegAddr));
        return -6746;
    }
